...
#          Filters out packages for AWS, Microsoft Azure, Google Cloud Platform, IBM Cloud, and Oracle Cloud.
...
#   - Filters cloud provider packages: AWS, Azure, GCP, IBM Cloud, Oracle Cloud
#
...
#   export PUBLISH_PREFIX=no-cloud-security       # Default: no-cloud-security
...
EMAIL_RECIPIENT="${EMAIL_RECIPIENT:-msuarladmin@msuarl.org}"
EMAIL_SENDER="${EMAIL_SENDER:-msuarladmin@msuarl.org}"
...
APTLY_ROOT_DIR="${APTLY_ROOT_DIR:-$(grep -o '"rootDir"\s*:\s*"[^"]*"' /etc/aptly/aptly.conf | cut -d'"' -f4)}"
if [[ -z "$APTLY_ROOT_DIR" ]]; then
  APTLY_ROOT_DIR="/var/lib/aptly"
  echo -e " ${YELLOW}⚠${NC} Could not parse rootDir from /etc/aptly/aptly.conf, using default: $APTLY_ROOT_DIR"
else
  echo -e " ${GREEN}✓${NC} Parsed rootDir: $APTLY_ROOT_DIR"
fi

PUBLISH_ROOT="${PUBLISH_ROOT:-$APTLY_ROOT_DIR/public}"
PUBLISH_PREFIX="${PUBLISH_PREFIX:-no-cloud-security}"
PUBLISH_DIR="$PUBLISH_ROOT/$PUBLISH_PREFIX"

# ─────────────────────────────────────────────────────────────
# Interactive publication directory configuration
# ─────────────────────────────────────────────────────────────
echo "=== Configure publication directory ==="
echo "Default configuration:"
echo "  PUBLISH_ROOT: $PUBLISH_ROOT"
echo "  PUBLISH_PREFIX: $PUBLISH_PREFIX"
echo "  Full path: $PUBLISH_DIR"
echo

# Prompt user to customize or accept defaults
read -p "Use these defaults? (y/n): " -r CONFIG_CONFIRM
if [[ ! $CONFIG_CONFIRM =~ ^[Yy]$ ]]; then
  echo
  echo "Configuration options:"
  echo "  1) Change PUBLISH_PREFIX only (e.g., 'no-cloud-security' → 'custom-prefix')"
  echo "  2) Change PUBLISH_ROOT only (e.g., '/var/lib/aptly/public' → '/custom/path')"
  echo "  3) Change both PUBLISH_ROOT and PUBLISH_PREFIX"
  echo

  read -p "Select option (1-3): " -r CONFIG_OPTION

  case "$CONFIG_OPTION" in
    1)
      read -p "Enter new PUBLISH_PREFIX: " -r NEW_PREFIX
      if [[ -n "$NEW_PREFIX" ]]; then
        PUBLISH_PREFIX="$NEW_PREFIX"
      fi
      ;;
    2)
      read -p "Enter new PUBLISH_ROOT directory: " -r NEW_ROOT
      if [[ -n "$NEW_ROOT" ]]; then
        PUBLISH_ROOT="$NEW_ROOT"
      fi
      ;;
    3)
      read -p "Enter new PUBLISH_ROOT directory: " -r NEW_ROOT
      ...
      ;;
    *)
      echo "Invalid option, using defaults"
      ;;
  esac
...
if [[ "$PUBLISH_PREFIX" =~ [';$`'] ]]; then
  error_exit "PUBLISH_PREFIX contains potentially dangerous characters"
fi

echo "✓ Publication directory confirmed: $PUBLISH_DIR"
echo
...
echo " Note: CLOUD PROVIDER FILTERS APPLIED"
echo " Filtering out: AWS, Microsoft Azure, Google Cloud Platform, IBM Cloud, Oracle Cloud"
echo " Suite: noble-security"
echo " Components: main, restricted, universe, multiverse (separate mirrors)"
echo

# Cloud provider package filter
# Excludes packages from: AWS, Azure, GCP, IBM Cloud, and Oracle Cloud
CLOUD_FILTER='!(Package (~ "^aws-" | ~ "^amazon-" | ~ "^ec2-" | ~ "^azure-" | ~ "walinuxagent" | ~ "^google-" | ~ "^gce-" | ~ "^google-cloud-" | ~ "^ibm-" | ~ "^ibm-cloud-" | ~ "^oracle-" | ~ "^oracle-cloud-" | ~ "^oci-"))'
...
for MIRROR in ubuntu-noble-security-main ubuntu-noble-security-restricted ubuntu-noble-security-universe ubuntu-noble-security-multiverse; do
  if $APTLY_CMD mirror show "$MIRROR" &>/dev/null; then
    PKG_COUNT=$($APTLY_CMD mirror show "$MIRROR" | grep 'Number of packages' | awk '{print $4}')
...
echo "Cloud Filtering: ENABLED (AWS, Azure, GCP, IBM Cloud, Oracle Cloud excluded)"
...
echo "Note: This script publishes to 'no-cloud-security' prefix for segregation from general repositories."
echo "The no-cloud-security repository contains only security updates with cloud provider packages filtered."
echo "Options for deployment:"
echo "  1. Use as standalone APT source: Point clients to http://<server>/no-cloud-security"
echo "  2. Keep separate from main ubuntu repository: Clients add both ubuntu and no-cloud-security sources"
echo "  3. Can be served independently for air-gapped or cloud-restricted environments"
echo

echo "=== Deployment Instructions ==="
echo
echo "Add to APT (DEB822 format in /etc/apt/sources.d/no-cloud-security.sources):"
echo "  Types: deb"
echo "       URIs: http://<server-ip>/no-cloud-security"
echo "       Suites: noble-security"
echo "       Components: main restricted universe multiverse"
echo "       Trusted: yes"
echo
echo "Or use traditional format in /etc/apt/sources.list:"
echo "  deb [trusted=yes] http://<server-ip>/no-cloud-security noble-security main restricted universe multiverse"
echo
